...
#
# Usage:
#   ./build-linux-arm64.sh           # Build with SDL3 GUI (default)
#   ./build-linux-arm64.sh headless  # Build headless (no display)
#
...
if [[ ! -x "$ROOT/Linux/arm64/bin/mk" ]]; then
    ...
    if [[ -f "getcallerpc-Linux-arm64.S" ]]; then
        ...
        gcc -c getcallerpc-Linux-arm64.S -o getcallerpc-Linux-arm64.o
    fi
    ...
    echo "  Linking mk..."
    gcc -fcommon -o mk *.o -L"$ROOT/Linux/arm64/lib" -lregexp -lbio -l9

    strip mk
    cp mk "$ROOT/Linux/arm64/bin/"
    echo "mk installed to $ROOT/Linux/arm64/bin/mk"
    cd "$ROOT"
    echo ""
fi
...
echo ""
echo "mk is available at: $ROOT/Linux/arm64/bin/mk"
echo ""
...
for lib in lib9 libbio libmp libsec libmath libfreetype libmemdraw libmemlayer libdraw; do
    if [[ -d "$ROOT/$lib" ]]; then
        echo "Building $lib..."
        cd "$ROOT/$lib"
        "$ROOT/Linux/arm64/bin/mk" install 2>&1 || { echo "ERROR: $lib build failed" >&2; exit 1; }
    fi
done
...
cd "$ROOT/limbo"
"$ROOT/Linux/arm64/bin/mk" install 2>&1 || { echo "ERROR: limbo build failed" >&2; exit 1; }
...
strip "$ROOT/Linux/arm64/bin/limbo"
...
if [[ "$GUIMODE" == "headless" ]]; then
    ...
    "$ROOT/Linux/arm64/bin/mk" -f mkfile-g 2>&1 || { echo "ERROR: emulator build failed" >&2; exit 1; }
else
    # SDL3 GUI build: main mkfile defaults to GUIBACK=sdl3
    rm -f emu.c errstr.h 2>/dev/null
    "$ROOT/Linux/arm64/bin/mk" 2>&1 || { echo "ERROR: emulator build failed" >&2; exit 1; }
fi
...
for f in tools9p.dis agentlib.dis; do
