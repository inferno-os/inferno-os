...
write_key() {
    # $1 = service name (anthropic, openai, brave)
    # $2 = key value
...
setup_ollama() {
    ...
    # llmsrv uses: -b openai -u http://localhost:11434/v1 -M <model>
    mkdir -p "$ROOT/lib/veltro"
    cat > "$ROOT/lib/veltro/llm.cfg" <<EOF
# Veltro LLM configuration — generated by setup-macos.sh
backend=openai
...
EOF
    ...
    printf "${DIM}  Start it with: ollama serve${RESET}\n"
}
...
# Detect emulator
EMU=""
if [ -x "$ROOT/emu/MacOSX/o.emu" ]; then
    EMU="./emu/MacOSX/o.emu -r."
fi
...
if [ -n "$EMU" ]; then
    ...
    printf "  ./build-macos-sdl3.sh   ${DIM}(GUI with SDL3)${RESET}\n"
    printf "  ./build-macos-headless.sh ${DIM}(terminal only)${RESET}\n"
fi
printf "\n"
