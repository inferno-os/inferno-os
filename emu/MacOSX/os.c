...
#ifdef GUI_SDL3
/*
 * Wrapper for emuinit when run on worker thread
 * Sets up thread-local Proc before calling emuinit
 */
static void*
emuinit_worker(void *arg)
{
    ...
    if(pthread_setspecific(prdakey, p))
        panic("emuinit_worker: pthread_setspecific failed");
...
libinit(char *imod)
{
    ...
    /* Use emuinit_worker wrapper - sets up Proc for worker thread */
