/*
 * lib9.h - Windows AMD64 platform types and declarations
 *
 * Based on Nt/386/include/lib9.h, updated for 64-bit Windows (LLP64).
 *
 * Windows x64 (LLP64) type sizes:
 *   int    = 32 bits
 *   long   = 32 bits  (NOT 64 bits like Linux LP64!)
 *   void*  = 64 bits
 *   long long = 64 bits
 *
 * We define ulong as 'unsigned long long' to match the 64-bit semantics
 * expected by Inferno's portable code (which assumes LP64 on 64-bit).
 */
...
/*
 * CRITICAL: On Windows x64 (LLP64), 'unsigned long' is only 32 bits.
 * Inferno's portable code expects 'ulong' to be pointer-sized (64 bits
 * on 64-bit platforms). We use 'unsigned long long' to match this.
 */
...
/* strdup is already declared by <string.h> on MSVC */
...
/*
 * malloc
 * Note: malloc, free, calloc, realloc are already declared by <stdlib.h>
 * with __declspec(dllimport) on MSVC. Don't re-declare them.
 */
