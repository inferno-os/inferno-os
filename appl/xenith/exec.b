...
{
	if(et != nil && et.w != nil)
		warning(nil, sprint("/mnt/xenith/%d/\n", et.w.id));
}
...
fontx(et : ref Text, t : ref Text, argt : ref Text, arg : string, narg : int)
{
	...
	if(newfont != nil){
		graph->draw(gui->mainwin, t.w.r, xenith->textcols[Framem->BACK], nil, (0, 0));
		t.reffont.close();
		t.reffont = newfont;
...
run(win : ref Window, s : string, rdir : string, ndir : int, newns : int, argaddr : string, arg : string, iseditcmd: int)
{
	...
	if(newns){
		...
		if(c.md == nil){
			warning(nil, "can't mount /mnt/xenith");
			exit;
		}
		if(winid > 0 && (pipechar=='|' || pipechar=='>')){
			buf := sys->sprint("/mnt/xenith/%d/rdsel", winid);
		...
		if((winid > 0 || iseditcmd) && (pipechar=='|' || pipechar=='<')){
			...
			if(iseditcmd){
				if(winid > 0)
					buf = sprint("/mnt/xenith/%d/editout", winid);
				else
 					buf = sprint("/mnt/xenith/editout");
			}
			else
				buf = sys->sprint("/mnt/xenith/%d/wrsel", winid);
	...
	if(argaddr != nil)
		utils->setenv("xenithaddr", argaddr);
	hard := 0;
...
# (or ,> or ,<) to lock up xenith.  Easy fix.  Add these two lines
# to the failure case of runwaittask():
#
# /sys/src/cmd/xenith/exec.c:1287 a exec.c:1288,1289
# else{
# if(c->iseditcmd)
# sendul(cedit, 0);
# free(c->name);
