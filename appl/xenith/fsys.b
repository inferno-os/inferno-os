...
dirtab := array[10] of {
	...
	Dirtab ( "xenith",	QTDIR,		Qxenith,		8r500|DMDIR ),
...
dirtabw := array[15] of {
	...
	Dirtab ( "colors",	QTFILE,		QWcolors,		8r600 ),
...
fsysmount(dir : string, ndir : int, incl : array of string, nincl : int) : ref Mntdir
{
	...
	if(sys->mount(cfd, nil, "/mnt/xenith", Sys->MREPL, buf) < 0){
		fsysdelid(m);
		return nil;
	}
	# cfd = nil;
	sys->bind("/mnt/xenith", "/chan", Sys->MBEFORE);	# was MREPL
	if(sys->bind("/mnt/xenith", "/dev", Sys->MBEFORE) < 0){
		fsysdelid(m);
		return nil;
	}
	return m;
...
fsysversion(x : ref Xfid) : ref Xfid
{
	...
	return respond(x, t, "xenith: bad version");
...
fsysauth(x : ref Xfid) : ref Xfid
{
	...
	return respond(x, t, "xenith: authentication not required");
}
	...
	if(nwname > 0){
		for(i = 0; i < nwname; i++){
			...
			if (!regular) {
				...
				# if(FILE(f.qid) == Qxenith) 	# empty directory
				#	break;
				if(name == "new"){
					if(w != nil)
...
fsysread(x : ref Xfid, f : ref Fid) : ref Xfid
{
	...
	if(f.qid.qtype & QTDIR){
		...
		if(FILE(f.qid) == Qxenith){	# empty dir
			t.data = nil;
			t.count = 0;
			respond(x, t, nil);
			return x;
